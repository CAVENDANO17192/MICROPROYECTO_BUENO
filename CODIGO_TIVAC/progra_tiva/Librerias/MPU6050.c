...
void MPU6050INIT(void)
{
    ...
    MPU6050Init(&sMPU6050, &g_sI2CMSimpleInst, 0x68, MPU6050Callback, &sMPU6050);//se activa el modulo MPU6050
                                                                                 ...
                                                                                 //El tercer valor es la direccion del MPU6050
    ...
    MPU6050ReadModifyWrite(&sMPU6050, MPU6050_O_ACCEL_CONFIG, ~MPU6050_ACCEL_CONFIG_AFS_SEL_M,
        MPU6050_ACCEL_CONFIG_AFS_SEL_4G, MPU6050Callback, &sMPU6050);//Se configura ACCELEROMETRO
                                                                     ...
                                                                     //El segundo valor es la direccion del registro que se desea configurar
...
void MPU6050READDATA (struct DATOS_STRUCT *self)
{
    ...
    self->ax = (double)fAccel[0];
    self->ay = (double)fAccel[1];
    self->az = (double)fAccel[2];

    self->gx = (double)fGyro[0];
    self->gy= (double)fGyro[1];
    self->gz= (double)fGyro[2];

    self->Accx = (atan2(fAccel[0], sqrt (fAccel[1] * fAccel[1] + fAccel[2] * fAccel[2]))*180.0)/3.14;//Es el giro(roll) en el eje x
    self->Accy = (atan2(fAccel[1], sqrt (fAccel[0] * fAccel[0] + fAccel[2] * fAccel[2]))*180.0)/3.14;//es el giro(roll) en el eje y

    return self;
}
void filtro_complementario_posicion(struct DATOS_STRUCT *self, double Filtrox, double gyrox, double Accx, double Filtroy, double gyroy, double Accy, double dt, double alpha)
{
    self->Filtrox = alpha*(Filtrox + gyrox * dt) + (1-alpha) * Accx;
    self->Filtroy = alpha*(Filtroy + gyroy * dt) + (1-alpha) * Accy;
    return self;
}
void filtro_complementario_velocidad(struct DATOS_STRUCT *self, double Velocidadx, double gyrox, double Velocidady, double gyroy, double Velocidadz, double gyroz, double alpha)
{
    self->Velocidadx = (1-alpha)*Velocidadx + alpha*gyrox;
    self->Velocidady = (1-alpha)*Velocidady + alpha*gyroy;
    self->Velocidadz = (1-alpha)*Velocidadz + alpha*gyroz;
    return self;
}
